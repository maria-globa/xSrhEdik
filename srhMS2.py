...
class SrhMs2():
    ...
    def initDataTable(self, srhFits, frequencyChannel, ewLcpPhaseCorrection, ewRcpPhaseCorrection, sLcpPhaseCorrection, sRcpPhaseCorrection):
        ...
        for scan in range(srhFits.dataLength):
            ...
            for vis in range(visibilityNumber):
                ...
                i = vis // 32
                j = vis % 32 
                uv[0,0] = srhFits.visLcp[frequencyChannel,scan,vis] * NP.exp(1j*(srhFits.ewAntPhaLcp[j + 1] - srhFits.sAntPhaLcp[i + 1] + ewLcpPhaseCorrection[j] + sLcpPhaseCorrection[i]))# / (srhFits.ewAntAmpLcp[j + 1] * srhFits.sAntAmpLcp[i + 1])
                uv[0,1] = srhFits.visRcp[frequencyChannel,scan,vis] * NP.exp(1j*(srhFits.ewAntPhaRcp[j + 1] - srhFits.sAntPhaRcp[i + 1] + ewRcpPhaseCorrection[j] + sRcpPhaseCorrection[i]))# / (srhFits.ewAntAmpRcp[j + 1] * srhFits.sAntAmpRcp[i + 1])
